...
double cut_eta_bjet(3.0); //pseudo-rapidity cut for b-jets
...
int main(int argc, char *argv[]) {
  ...
  for(int ii = minevents; ii < maxevents; ii++) {
	...
	Jets_unsort.push_back(smearer_j_CMS_Benj(jetcan.e(), jetcan.perp(), jetcan.pz(),
	 ...
	 //Set the charge
	 bJets_unsort[jj].set_user_index(thebJets[4][jj]);
       }
     }
     ...
     if(bJet1.perp() > cut_pt_bjet1 && bJet2.perp() > cut_pt_bjet2 && bJet3.perp() > cut_pt_bjet3 && bJet4.perp() > cut_pt_bjet3 && bJet5.perp() > cut_pt_bjet3 && bJet6.perp() > cut_pt_bjet3 && passed_all_cuts) {
       pass_ptb+=evweight;
     }
     else {
       passed_all_cuts = false;
     ...
     if(deltaR(bJets[pairs_of_six[mincombo][0]], bJets[pairs_of_six[mincombo][1]]) < cut_dR_hbbreco && deltaR(bJets[pairs_of_six[mincombo][2]], bJets[pairs_of_six[mincombo][3]]) < cut_dR_hbbreco && deltaR(bJets[pairs_of_six[mincombo][4]], bJets[pairs_of_six[mincombo][5]]) < cut_dR_hbbreco && passed_all_cuts) { pass_dRbbhiggses+=evweight;
     }
     else {
       passed_all_cuts = false;
     }
